...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        if SerialObj:
            try:
                SerialObj.write(f"Elbow: {int(right_elbow_angle)}, Shoulder: {int(right_shoulder_angle)}\n".encode())
            except Exception as e:
                print(f"Serial transmission error: {e}")
        # print(f"Elbow: {int(right_elbow_angle)}, Shoulder: {int(right_shoulder_angle)}\n".encode())
        ...
        cv2.circle(frame, tuple(right_hip), 5, (255, 255, 0), -1)
        ...
        cv2.line(frame, tuple(right_hip), tuple(right_shoulder), (255, 255, 0), 2)
        cv2.putText(frame, f"Elbow Angle: {int(right_elbow_angle)} deg", tuple(right_elbow),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        cv2.putText(frame, f"Shoulder Angle: {int(right_shoulder_angle)} deg", tuple(right_shoulder),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

    cv2.imshow("Elbow and Shoulder Angles", frame)
